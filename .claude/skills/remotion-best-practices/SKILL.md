...
## SVG infographic in Remotion

When you need to display an existing static SVG infographic (e.g. a file produced by the `infographic-creator` skill at `output/blockscore/infographics/*.svg`) inside a video, load [rules/svg-infographic.md](rules/svg-infographic.md). It covers four patterns: simple `<Img>` fade/scale, pan/zoom (Ken Burns) over tall portrait infographics, full inline injection with per-element animation via `dangerouslySetInnerHTML` + `useRef`, and clip-path top-to-bottom reveal. Includes ready-to-paste compositions for reel (1080×1920), square (1080×1080), and hero (1920×1080) framings, plus a `useStaticSvg` hook that wraps `delayRender`/`continueRender` correctly.

## BlockScore brand bindings

When building any video for the BlockScore brand (project name, prop, or content references BlockScore / score gauge / anonymous tenant feedback), load [rules/blockscore-brand.md](rules/blockscore-brand.md). It binds the canonical design tokens at `.claude/skills/_shared/blockscore/` (`tokens.md`, `tokens.json`) into Remotion-ready TS constants, font loaders (`@remotion/google-fonts/InterTight`, `Inter`, `GeistMono`), drop-in components (`<BlockScoreBackground>`, `<BlockScoreWordmark>`, `<ScoreGauge>`, `<Emph>`), composition presets (Reel 1080×1920, Square 1080×1080, Hero 1920×1080, OG 1200×630), and brand-correct motion easing. Never hardcode brand hex codes — import from `src/brand/blockscore/tokens.ts` instead.
